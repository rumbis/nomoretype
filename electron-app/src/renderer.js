/* ═══════════════════════════════════════════════════════════════════════════
   Renderer — Main UI logic for NoMoreType Electron
   ═══════════════════════════════════════════════════════════════════════════ */
...
function initNavigation() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tabName === 'history' && window._refreshHistory) {
...
function initFileTab() {
  ...
  dropZone.addEventListener('drop', async (e) => {
    e.preventDefault();
    dropZone.classList.remove('drag-over');
    showToast('Please click to select a file (drag path unsupported in Electron)', 'warning');
  });
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    if (savePath) {
      await window.electronAPI.writeFile(savePath, resultText.textContent);
      showToast(`Saved!`);
    }
  });
}
...
function initMicTab() {
  ...
  saveBtn.addEventListener('click', async () => {
    const savePath = await window.electronAPI.saveFileDialog('mic_transcript.txt');
    if (savePath) {
      await window.electronAPI.writeFile(savePath, resultText.textContent);
      showToast('Saved!');
    }
...
function initYouTubeTab() {
  ...
  getSubsBtn.addEventListener('click', async () => {
    ...
    if (!window.youTubeService.isAvailable()) {
      showToast('yt-dlp not found. Install: brew install yt-dlp', 'error', 5000);
      return;
    }
  ...
  saveBtn.addEventListener('click', async () => {
    const savePath = await window.electronAPI.saveFileDialog('youtube_transcript.txt');
    if (savePath) {
      await window.electronAPI.writeFile(savePath, resultText.textContent);
      showToast('Saved!');
    }
...
async function startYouTubeService() {
  const ytPath = await window.youTubeService.init();
  if (ytPath) console.log('yt-dlp ready at', ytPath);
}
...
function initHistoryTab() {
  ...
  modalCopy.addEventListener('click', () => {
    navigator.clipboard.writeText(modalText.textContent).then(() => showToast('Copied!'));
  });
  ...
  exportBtn.addEventListener('click', async () => {
    ...
    const savePath = await window.electronAPI.saveFileDialog('transcription_history.json');
    if (savePath) {
